...
with open('merge_500.dwdb') as file:
...
while trace_stop < test_trace + 1:
    ...
    #while i < len(meta):
        ...
        trace = dwdb_reader.read_trace(meta[i]['filename'])
...
# print(p_value)
    ...
    tr = np.array(p_value)
    Dpaws.TraceWrite(tr, tracefile='chi_result/chi_revolution_p/yuan_chi_p{}.dwfm'.format(trace_stop))
    p_value2 = []
    small_value = 1e-323
    ...
    Dpaws.TraceWrite(tr, tracefile='chi_result/chi_revolution_plog/yuan_chi_plog{}.dwfm'.format(trace_stop))
    trace_start = trace_start +step
    trace_stop =  trace_stop + step

np.savetxt("chi_result/leak_count.txt", overthr_count, fmt='%s')
np.savetxt("chi_result/leak_max.txt", max, fmt='%s')
