...
from scipy.stats import chi2, distributions
...
for b in range(0, bootnum):
    ...
    random_list = random_list % len(rand)

	# FIX: classification to boot_rand or boot_fix
	cls = np.random.randint(2, size=len(random_list))
	
	boot_fix = fix[random_list[a==0]]
	boot_rand = fix[random_list[a==1]]
	...
	boot_rand  = rand[random_list[a==0]]
	boot_rand2 = rand2[random_list[a==1]]
...
# QUESTION: If parameters of the uniform distributions are known
# Why don't recreate it on the fly instead of reading as np.random.uniform(0,1,bootnum)?
